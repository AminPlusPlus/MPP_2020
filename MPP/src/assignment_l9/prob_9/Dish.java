...
public class Dish {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(menu.stream().anyMatch(v->v.isVegetarian()));

        //b. Is there any healthy menu have calories less than 1000 ( return type boolean)
        System.out.println(menu.stream().anyMatch(c->c.getCalories()<1000));

        //c. Is there any unhealthy menu have calories greater than 1000 ( return type boolean)
        System.out.println(menu.stream().anyMatch(s->s.getCalories()>1000));
        ...
        System.out.println(menu.stream().map(t->t.getCalories())
                .reduce((a,b)-> Dish.calculateTotalCalories(a,b)).get());

        //f. calculateTotalCaloriesMethodReference()in the menu using MethodReferences. (return int)
        System.out.println(menu.stream().map(t->t.getCalories())
                .reduce(Dish::calculateTotalCalories).get());


    }


    static int calculateTotalCalories(int a,int b) {
        return a+b;
    }
}